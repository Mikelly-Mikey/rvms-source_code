...
async function applyPaymentToBooking(booking, payment) {
  ...
  try {
    const customer =
      booking.customer || (await Customer.findByPk(booking.customer_id));
...
router.post(
  ...
  [
    ...
    body("method")
      .isIn(["cash", "mpesa", "bank_transfer", "card"])
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // --- SIMULATION TRIGGER ---
      // If in simulation mode, automatically fire the callback after a delay
      if (result.simulated) {
        const delay = parseInt(process.env.MPESA_SIM_DELAY_MS || "4000", 10);
        const port = process.env.PORT || 3000;
        const callbackUrl =
          process.env.MPESA_CALLBACK_URL ||
          `http://localhost:${port}/payments/mpesa/callback`;

        console.log(
          `[SIMULATOR] Scheduling automatic callback in ${delay}ms...`,
        );
        ...
        setTimeout(async () => {
          try {
                ...
                stkCallback: {
                  ...
                  CallbackMetadata: {
                    Item: [
                      ...
                      { Name: "PhoneNumber", Value: result.phone },
                    ],
                  },
                },
              },
            ...
            console.log(
              `[SIMULATOR] Automatic callback successfully sent for ${result.checkoutRequestId}`,
            );
          } catch (err) {
            console.error(
              "[SIMULATOR] Automatic callback trigger failed:",
              err.message,
            );
          }
        }, delay);
      ...
      return res.json({
        ...
        checkoutRequestId: result.checkoutRequestId,
        message:
          result.customerMessage || "STK push sent. Awaiting confirmation...",
      });
    } catch (error) {
...
router.post("/mpesa/callback", async (req, res) => {
  ...
  try {
    console.log(
      "[M-PESA CALLBACK] Received payload:",
    ...
    if (!parsed.success) {
      console.log(
        `[M-PESA CALLBACK] Transaction ${parsed.checkoutRequestId} FAILED: ${parsed.resultDesc}`,
      );
      await tx.update({
    ...
    console.log(
      `[M-PESA CALLBACK] Transaction ${parsed.checkoutRequestId} SUCCESS. Receipt: ${parsed.receipt}`,
    );
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ success: false, message: "Error fetching status" });
    }
  },
);
