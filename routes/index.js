...
router.get('/', requireAuth, async (req, res) => {
  ...
  if (req.user.role === 'admin') {
    try {
      ...
      return res.render('admin-dashboard', { user: req.user, ...dashboardData });
    } catch (error) {
      console.error('Error loading admin dashboard:', error);
      return res.status(500).render('error', { message: 'Error loading admin dashboard' });
    }
  }
  try {
  ...
  try {
    ...
    if (!customer) {
      return res.render('customer-dashboard', {
        user: req.user,
        available_vehicles: await Vehicle.findAll({ where: { status: 'available' }, order: [['registration', 'ASC']] }),
        user_bookings: [],
        completed_bookings: [],
        payments: [],
        total_spent: 0,
        profile_message: 'Your account is not linked to a customer profile yet. Please contact reception.',
      });
    }
    ...
    const completedBookings = await Booking.findAll({
      where: { customer_id: customer.customer_id, status: ['completed', 'checked-out'] },
      include: ['customer', 'vehicle'],
      order: [['created_date', 'DESC']],
    });
