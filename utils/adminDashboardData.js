...
function getPeriodRange(period) {
  ...
  return {
    ...
    period: period || "daily",
    label: labels[period] || labels.daily,
  };
}
  ...
  if (period === "daily") {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      data.push(
        payments
          .filter(
            (p) => new Date(p.payment_date).toISOString().slice(0, 10) === key,
          )
  ...
  for (let i = 11; i >= 0; i--) {
    ...
    labels.push(
      month.toLocaleDateString("en-US", {
        month: "short",
        year: period === "annual" ? "2-digit" : undefined,
      }),
    );
    data.push(
      payments
...
async function getAdminDashboardData(period = "daily") {
  ...
  const [
    ...
    Payment.sum("amount", {
      where: { payment_date: dateFilter, status: "completed" },
    }),
    ...
    Booking.count({
      where: { status: "completed", balance_due: { [Op.gt]: 0 } },
    }),
    Vehicle.count({ where: { status: "maintenance" } }),
    MaintenanceSchedule.count({
      where: { status: "pending", scheduled_date: { [Op.lte]: end } },
    }),
    MaintenanceSchedule.count({
      where: { status: "pending", service_type: "inspection" },
    }),
  ...
  if (completedBookingsInPeriod.length > 0) {
    ...
    const totalRev = completedBookingsInPeriod.reduce(
      (sum, b) => sum + parseFloat(b.total_amount || 0),
      0,
    );
    avgRevenuePerBooking = Math.round(
      totalRev / completedBookingsInPeriod.length,
    );
  }
  ...
  const avgRating =
    avgRatingResult.length > 0
      ? (
          avgRatingResult.reduce((sum, r) => sum + r.rating, 0) /
  ...
  const collectionRate =
    pendingInvoices + paymentsPeriod > 0
  ...
  if (fleetMaintenancePending > 0) {
    alerts.push({
      ...
      message: `${fleetMaintenancePending} alert(s) require attention`,
      created_at: new Date(),
    });
  }
  if (pendingReviews > 0) {
    alerts.push({
      ...
      message: `${pendingReviews} review(s) awaiting approval`,
      created_at: new Date(),
    });
  }
  if (receptionPendingReturns > 0) {
    alerts.push({
      ...
      message: `${receptionPendingReturns} vehicle(s) awaiting return`,
      created_at: new Date(),
    });
  }
  ...
  return {
    ...
    fleet_supervisor_name: fleetSupervisor
      ? fleetSupervisor.getFullName()
      : "Not Assigned",
